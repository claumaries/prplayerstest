<h1 align="center">
Test for Senior Laravel/PHP Developer
</h1>

<br>

This test is designed to test your knowledge of PHP Design principles and its implementations on the Laravel framework. It is divided into 3 levels, with each feature having to be accomplished consecutively.

**Test Point System**: The test total is 125 points with additional 32 bonus points.
...
**Test Duration**: You are given 3 days to complete the test. On a separate spreadsheet, please log the time spent per functionality (start time and end time).

**Output**: You are expected to send an email with an attachment of your output files (preferably a zip file). Alternatively, you can attach a link to your GitHub Repository, Google Drive, Dropbox, or any other storage service to download the output if the file is too large to be attached on email, or for other reason.
...
- **+1 point** - Style the pages using a preferred framework (e.g. tailwindcss, bootstrap, vuetify, etc.).
...
- [ ] Write Unit testing for the service class (You can use the `phpunit` package or Laravel's pest package)
